...
TEST_F(MeanSquareErrorTest, Initialization) {
  ...
  error_layer_ = MeanSquareError(&input_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  EXPECT_TRUE(error_layer_.Init());
}

TEST_F(MeanSquareErrorTest, DerivativeTest) {
  error_layer_ = MeanSquareError(&input_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  error_layer_.Init();
...
TEST_F(MeanSquareErrorTest, GradientChecking) {
  error_layer_ = MeanSquareError(&input_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  error_layer_.Init();
...
TEST_F(MeanSquareErrorTest, Convergence) {
  error_layer_ = MeanSquareError(&input_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  error_layer_.Init();
