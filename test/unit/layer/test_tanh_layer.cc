...
TEST_F(TanhLayerTest, Initialization) {
  ...
  tanh_layer_ = TanhLayer(&input_layer_);
  error_layer_ = MeanSquareError(&tanh_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  EXPECT_TRUE(error_layer_.Init());
}

TEST_F(TanhLayerTest, DerivativeTest) {
  tanh_layer_ = TanhLayer(&input_layer_);
  error_layer_ = MeanSquareError(&tanh_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  error_layer_.Init();
