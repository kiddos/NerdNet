...
using nerd::nn::SigmoidCrossEntropy;
using nerd::nn::Tensor;
using arma::Mat;
using arma::Row;

class SigmoidCrossEntropyTest : public ::testing::Test {
 public:
  enum { INPUT_SIZE = 2, OUTPUT_SIZE = 6, DATA_SIZE = 4 };

  SigmoidCrossEntropyTest()
 ...
 protected:
  ...
  SigmoidCrossEntropy error_layer_;

  arma::Mat<float> test_data_;
  arma::Mat<float> test_label_;
};

TEST_F(SigmoidCrossEntropyTest, Initialization) {
  ...
  error_layer_ = SigmoidCrossEntropy(&input_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  EXPECT_TRUE(error_layer_.Init());
}

TEST_F(SigmoidCrossEntropyTest, DerivativeTest) {
  error_layer_ = SigmoidCrossEntropy(&input_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  error_layer_.Init();
...
TEST_F(SigmoidCrossEntropyTest, GradientChecking) {
  error_layer_ = SigmoidCrossEntropy(&input_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  error_layer_.Init();
...
TEST_F(SigmoidCrossEntropyTest, Convergence) {
  error_layer_ = SigmoidCrossEntropy(&input_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  error_layer_.Init();
