...
class LeakyReluLayerTest : public ::testing::Test {
 ...
 protected:
  ...
  LeakyReluLayer leaky_relu_layer_;
...
TEST_F(LeakyReluLayerTest, Initialization) {
  ...
  leaky_relu_layer_ = LeakyReluLayer(&input_layer_);
  error_layer_ =
      MeanSquareError(&leaky_relu_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  EXPECT_TRUE(error_layer_.Init());
}

TEST_F(LeakyReluLayerTest, DerivativeTest) {
  leaky_relu_layer_ = LeakyReluLayer(&input_layer_);
  error_layer_ =
      MeanSquareError(&leaky_relu_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  error_layer_.Init();
