...
TEST_F(SigmoidLayerTest, Initialization) {
  ...
  sigmoid_layer_ = SigmoidLayer(&input_layer_);
  error_layer_ = MeanSquareError(&sigmoid_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  EXPECT_TRUE(error_layer_.Init());
}

TEST_F(SigmoidLayerTest, DerivativeTest) {
  sigmoid_layer_ = SigmoidLayer(&input_layer_);
  error_layer_ = MeanSquareError(&sigmoid_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  error_layer_.Init();
