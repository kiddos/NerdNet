...
TEST_F(ArctanLayerTest, Initialization) {
  ...
  arctan_layer_ = ArctanLayer(&input_layer_);
  error_layer_ = MeanSquareError(&arctan_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  EXPECT_TRUE(error_layer_.Init());
}

TEST_F(ArctanLayerTest, DerivativeTest) {
  arctan_layer_ = ArctanLayer(&input_layer_);
  error_layer_ = MeanSquareError(&arctan_layer_, {INPUT_SIZE, OUTPUT_SIZE});
  error_layer_.Init();
