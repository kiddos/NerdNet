...
Tensor<DType> reduce_mean(const Tensor<DType>& t, int axis) {
  ...
  if (axis == 0) {
    ...
    for (int i = 0 ; i < rows ; ++i) {
      ...
      output[i] = rowsum / cols;
    }
    return output;
  }
}

template float reduce_mean(const Tensor<float>& t);
template double reduce_mean(const Tensor<double>& t);
template Tensor<float> reduce_mean(const Tensor<float>& t, int axis);
template Tensor<double> reduce_mean(const Tensor<double>& t, int axis);
